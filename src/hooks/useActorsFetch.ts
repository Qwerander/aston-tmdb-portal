...
import {
  getActors,
  getActorsSearch,
} from '../store/reducers/actors/fetchMethods';
...
export function useActorsFetch() {
  ...
  const pagesPopular = Object.keys(
    useAppSelector((state) => state.actors.popular.actors)
  );
  const pagesSearch = Object.keys(
    useAppSelector((state) => state.actors.search.actors)
  );
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (valueSearch && valueSearch !== prevValueSearch.current) {
        ...
        !pagesSearch.includes(page.toString())
      ) {
